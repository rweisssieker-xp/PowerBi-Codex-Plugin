# Skills Catalog

The plugin contains more than 100 Codex skills. This catalog groups the most important skill families.

## Core Power BI

- `powerbi-reporting`
- `powerbi-modelling`
- `powerbi-dax`
- `powerbi-query-design`
- `powerbi-business-problem-factory`
- `powerbi-domain-packs`
- `powerbi-expert-review`
- `powerbi-delivery-factory`

## Expert Reduction and Operating Model

- `powerbi-requirement-to-model-autopilot`
- `powerbi-erp-object-resolver`
- `powerbi-kpi-conflict-war-room`
- `powerbi-executive-question-library`
- `powerbi-process-dax-pattern-library`
- `powerbi-auto-data-contract-generator`
- `powerbi-report-consolidation-ai`
- `powerbi-semantic-model-certification-bot`
- `powerbi-source-system-gap-detector`
- `powerbi-industrial-kpi-benchmark-pack`
- `powerbi-value-realization-tracker`
- `powerbi-report-ux-decision-coach`
- `powerbi-data-lineage-risk-scorer`
- `powerbi-sap-fabric-migration-playbooks`
- `powerbi-role-based-prompt-packs`
- `powerbi-closed-loop-action-roi`
- `powerbi-builder-skill-assessment`
- `powerbi-tenant-cleanup-autopilot`
- `powerbi-data-product-marketplace-generator`
- `powerbi-audit-ready-ai-evidence-trail`
- `powerbi-intake-router`
- `powerbi-portfolio-rationalization`
- `powerbi-kpi-governance`
- `powerbi-certified-dataset-factory`
- `powerbi-self-service-guardrails`
- `powerbi-performance-refresh-optimizer`
- `powerbi-fabric-architecture-advisor`
- `powerbi-migration-factory`
- `powerbi-cost-capacity-optimizer`
- `powerbi-business-owner-acceptance`
- `powerbi-coe-copilot`

## Industrial Process Intelligence

- `powerbi-industrial-process-intelligence`
- `powerbi-process-chain-kpi-visibility`
- `powerbi-source-system-process-adapter`
- `powerbi-industrial-root-cause-cockpit`
- `powerbi-erp-crm-system-catalog`
- `powerbi-enterprise-master-data-mapping`
- `powerbi-industry-vertical-packs`

## Governance, Quality, and Compliance

- `powerbi-regulatory-standards`
- `powerbi-compliance-evidence-pack`
- `powerbi-quality-gate`
- `powerbi-lineage-impact-analyzer`
- `powerbi-tenant-governance-scanner`
- `powerbi-data-contracts`
- `powerbi-business-glossary-generator`
- `powerbi-bilingual-documentation`

## Automation and Platform Engineering

- `powerbi-automated-intake-triage`
- `powerbi-automated-requirement-interview`
- `powerbi-automated-source-metadata-scan`
- `powerbi-automated-kpi-contract-generation`
- `powerbi-automated-semantic-model-drafting`
- `powerbi-automated-dax-test-generation`
- `powerbi-automated-powerquery-review`
- `powerbi-automated-report-ux-review`
- `powerbi-automated-refresh-failure-triage`
- `powerbi-automated-schema-drift-alerts`
- `powerbi-automated-tenant-cleanup`
- `powerbi-automated-certification-pipeline`
- `powerbi-automated-release-notes`
- `powerbi-automated-documentation-refresh`
- `powerbi-automated-uat-pack-builder`
- `powerbi-automated-action-creation`
- `powerbi-automated-adoption-monitoring`
- `powerbi-automated-capacity-optimization`
- `powerbi-automated-security-exposure-scan`
- `powerbi-automated-executive-briefing`
- `powerbi-live-metadata-scanner`
- `powerbi-pbip-tmdl-generator`
- `powerbi-dax-unit-test-runner`
- `powerbi-powerquery-code-generator`
- `powerbi-visual-layout-spec-generator`
- `powerbi-fabric-deployment-blueprint`
- `powerbi-connector-capability-matrix`
- `powerbi-enterprise-kpi-ontology`
- `powerbi-industry-solution-packs`
- `powerbi-system-object-dictionaries`
- `powerbi-golden-semantic-model-templates`
- `powerbi-automated-report-review-rubric`
- `powerbi-data-quality-rule-library`
- `powerbi-rls-ols-pattern-library`
- `powerbi-admin-api-playbook`
- `powerbi-synthetic-demo-data-generator`
- `powerbi-acceptance-test-pack-generator`
- `powerbi-migration-cost-estimator`
- `powerbi-coe-operating-handbook`
- `powerbi-machine-readable-skill-index`
- `powerbi-live-connector-toolkit`
- `powerbi-auto-semantic-model-builder`
- `powerbi-auto-report-layout-generator`
- `powerbi-dax-test-harness`
- `powerbi-powerquery-test-harness`
- `powerbi-semantic-model-regression`
- `powerbi-kpi-knowledge-graph`
- `powerbi-industrial-data-product-factory`
- `powerbi-action-recommendation-engine`
- `powerbi-closed-loop-bi`

## AI/KI Skills

- `powerbi-ai-kpi-discovery-agent`
- `powerbi-ai-business-question-generator`
- `powerbi-ai-semantic-model-refactor-agent`
- `powerbi-ai-measure-deduplication-agent`
- `powerbi-ai-dax-explainability-agent`
- `powerbi-ai-report-personalization-engine`
- `powerbi-ai-insight-prioritization-engine`
- `powerbi-ai-causal-hypothesis-tester`
- `powerbi-ai-data-story-generator`
- `powerbi-ai-exception-to-action-agent`
- `powerbi-ai-forecast-explanation-agent`
- `powerbi-ai-scenario-recommendation-agent`
- `powerbi-ai-data-contract-negotiator`
- `powerbi-ai-source-mapping-agent`
- `powerbi-ai-compliance-reviewer`
- `powerbi-ai-tenant-risk-predictor`
- `powerbi-ai-adoption-coach`
- `powerbi-ai-self-service-copilot-guard`
- `powerbi-ai-process-digital-twin-advisor`
- `powerbi-ai-expert-replacement-score`
- `powerbi-ai-continuous-improvement-agent`
- `powerbi-ai-bi-memory-agent`
- `powerbi-ai-governance-drift-detector`
- `powerbi-ai-schema-drift-resolver`
- `powerbi-ai-kpi-lifecycle-manager`
- `powerbi-ai-autonomous-remediation-agent`
- `powerbi-ai-business-impact-simulator`
- `powerbi-ai-decision-audit-agent`
- `powerbi-ai-data-product-owner-copilot`
- `powerbi-ai-report-retirement-agent`
- `powerbi-ai-semantic-contract-validator`
- `powerbi-ai-cross-process-dependency-mapper`
- `powerbi-ai-root-cause-playbook-generator`
- `powerbi-ai-enterprise-benchmark-learner`
- `powerbi-ai-report-test-case-miner`
- `powerbi-ai-data-steward-workbench`
- `powerbi-ai-security-exposure-analyzer`
- `powerbi-ai-capacity-optimization-agent`
- `powerbi-ai-multi-agent-bi-orchestrator`
- `powerbi-ai-boardroom-copilot`
- `powerbi-ai-copilot-orchestrator`
- `powerbi-rag-knowledge-base`
- `powerbi-ai-semantic-layer-translator`
- `powerbi-nl-to-dax-guardrails`
- `powerbi-nl-to-powerquery-generator`
- `powerbi-ai-report-reviewer`
- `powerbi-ai-data-steward`
- `powerbi-ai-kpi-negotiator`
- `powerbi-ai-process-advisor`
- `powerbi-ai-scenario-planner`
- `powerbi-ai-model-governance`
- `powerbi-ai-prompt-governance`
- `powerbi-ai-hallucination-guard`
- `powerbi-ai-insight-confidence`
- `powerbi-ai-root-cause-investigator`
- `powerbi-ai-auto-documentation`
- `powerbi-ai-training-coach`
- `powerbi-ai-change-impact-explainer`
- `powerbi-ai-ticket-resolver`
- `powerbi-ai-autonomous-bi-sprint`

## Enterprise Source Packs

- `powerbi-enterprise-software-source-catalog`
- `powerbi-epm-planning-source-pack`
- `powerbi-treasury-cash-source-pack`
- `powerbi-tax-compliance-audit-source-pack`
- `powerbi-esg-compliance-source-pack`
- `powerbi-qm-qms-ehs-source-pack`
- `powerbi-cx-marketing-commerce-source-pack`
- `powerbi-subscription-revenue-billing-source-pack`
- `powerbi-field-service-aftermarket-source-pack`
- `powerbi-procurement-supplier-risk-source-pack`
- `powerbi-logistics-visibility-source-pack`
- `powerbi-whs-wms-warehouse-source-pack`
- `powerbi-apm-observability-itops-source-pack`
- `powerbi-data-governance-mdm-source-pack`
- `powerbi-etl-integration-source-pack`
- `powerbi-data-quality-observability-source-pack`
- `powerbi-lakehouse-warehouse-source-pack`
- `powerbi-document-ecm-source-pack`
- `powerbi-ocr-idp-invoice-capture-source-pack`
- `powerbi-lowcode-workflow-rpa-source-pack`
- `powerbi-security-identity-source-pack`
